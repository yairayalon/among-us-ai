...
from CrewmateGameFlow import CrewmateGameFlow
from ImpostorGameFlow import ImpostorGameFlow
from Constants import *
from Crewmate import Crewmate
from GameRunnerHelper import GameRunnerHelper as Grh
from Impostor import Impostor
from Board import Board
import pandas as pd
from VoteFlow import *
from CrewmateNeuralNetwork import CrewmateNeuralNetwork
from ImpostorNeuralNetwork import ImpostorNeuralNetwork
...
class GameRunner:
    ...
    def init_game(is_gui):
        ...
        cm_obs, GameRunner.cm_desired, imp_obs, \
        GameRunner.imp_desired = Grh.get_obs_names()
    ...
    def run_game():
        ...
        if winning_team:
            ...
            cm_df.to_csv(
                f'crewmate_training{os.path.sep}crewmates{GameRunner.file_num}.csv')
            imp_df.to_csv(
                f'impostor_training{os.path.sep}impostors{GameRunner.file_num}.csv')
            ...
            if not agent.is_dead:
                if agent.agent_type:
                    if agent.decider_type == 'nn_dm':
                        ...
                        try:
                            agent_to_vote = impostor_vote(GameRunner.
                    ...
                    if agent.decider_type == 'nn_dm':
                        ...
                        try:
                            agent_to_vote = get_crew_vote(GameRunner.
                                board.get_observation(
                                agent),
                                living_agents,
                                GameRunner.cur_round,
