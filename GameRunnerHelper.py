...
class GameRunnerHelper:
    ...
    def get_obs_names():
        ...
        cm_obs = ['loc', 'time', 'time in round',
                   'round', 'num of agents in vision', 'table cooldown',
                   'table calls left', 'num tasks left', 'color', 'tasks left']
        imp_obs = ['loc', 'time', 'color', 'time in round',
                    'round', 'table cooldown', 'table calls left',
    ...
    def get_valid_dirs(agent_coord):
        return [direct for direct in directs if \
                GameRunnerHelper.is_valid_coord(
                    GameRunnerHelper.get_coord_after_move(
                        agent_coord, direct))]
