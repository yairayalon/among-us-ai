...
class ImpostorNeuralNetwork:
    ...
    def __from_pickle(self):
        infile = open("impostor", "rb")
        self.regr = pickle.load(infile)
        infile.close()

    def __to_pickle(self):
        out_file = open("impostor", 'wb')
        pickle.dump(self.regr, out_file)
        out_file.close()

    def __train_model(self):
        lst = [f"impostor_training{os.path.sep}{name}" for name in
               os.listdir('impostor_training')]
    ...
    def decide_action(self, observation, legal_actions):
        ...
        for action in legal_actions:
            ...
            res = ImpostorNeuralNetwork.__win_weight(result_vec[0][0],
                                                     cur_round) \
                  + ImpostorNeuralNetwork.__killed_weight(result_vec[0][1],
                                                          cur_round)
