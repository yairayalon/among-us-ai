...
def probs_voting_body(observation, colors, last_round, my_color):
    ...
    for c in colors:
        if observation \
                [f'killer according to reporter in round {last_round}'] == c:
            if c != my_color:
                killer = c
                break
    for c in colors:
...
def get_suspicion_table(observation, living_agents, my_color,
    ...
    for living in living_agents:
        ...
        summer = dist + 2 * tasks + percentage_seen + \
                 times_he_voted_for_me * 0.5 + 34000
...
def considerations(obs, last_round, myself, impostor_partner, agents):
    ...
    for round in range(1, last_round + 1):
        ...
        for agent in agents:
            ...
            dict_res[agent.color] = 4 * counter_for_myself \
                                    + counter_for_partner
