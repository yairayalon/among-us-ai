...
class ImpostorObservations:
    ...
    def __init__(self, observ_vertices, my_color, colors, impostors):
        ...
        self.tasks_faked = set()
        self.time_faked_tasks = 0
        for c in colors:
            if c in impostors:
                self.__obs[f'is impostor {c}'] = 1
            else:
                self.__obs[f'is impostor {c}'] = 0
            self.__obs[f'tasks num seen on {c}'] = 0
            self.__obs[f'tasks seen on {c}'] = set()
            self.__obs[f'is dead {c}'] = 0
            self.__obs[f'seen kill {c}'] = 0
            self.__obs[f'was seen killing by {c}'] = 0
