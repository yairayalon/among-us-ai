...
from CrewmateObservations import CrewmateObservations
from ImpostorObservations import ImpostorObservations
...
class Board:
    ...
    def __init__(self, length, width, agents, walls, tasks, table,
        ...
        impostors_num = len([agent for agent in agents if agent.agent_type])
    ...
    def board_update_impostor_observation(self, agent, total_cycles, cur_round,
                                          cycles_in_round):
        if cur_round >= 9:
            print(cur_round, 'WTFFFFFFFFFFFFFFFFFFF')
        observation = self.__agent_observations[agent]
