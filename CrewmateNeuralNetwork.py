...
from GameRunnerHelper import GameRunnerHelper
...
class CrewmateNeuralNetwork:
    ...
    def __from_pickle(self):
        infile = open("crewmate", "rb")
        self.regr = pickle.load(infile)
        infile.close()

    def __to_pickle(self):
        out_file = open("crewmate", 'wb')
        pickle.dump(self.regr, out_file)
        out_file.close()

    def __train_model(self):
        lst = [f"crewmate_training{os.path.sep}{name}" for name in
               os.listdir('crewmate_training')]
    ...
    def decide_action(self, observation, legal_actions):
        ...
        for action in legal_actions:
            ...
            res = CrewmateNeuralNetwork.__win_weight(result_vec[0][0], cur_round) \
                  + CrewmateNeuralNetwork.__tasks_weight(result_vec[0][1], cur_round)
